দুই বছর আগে নির্ভর কমিউনিকেশন শুরু করেছিল মাত্র দুটো পপ নিয়ে - মিরপুর এবং উত্তরা। প্রায় ৫ হাজার কাস্টমার। তখন তারা নটোবট সেটআপ করেছিল। সেই পরিকল্পনা অনুযায়ী কাজ করেছে, সফল হয়েছে।

এখন ২০২৬ সাল। নির্ভর কমিউনিকেশনের গ্রোথ হয়েছে বিশাল। বর্তমানে নর্থ ঢাকায় তাদের আটটা পপ, প্রায় পঞ্চাশ হাজার কাস্টমার। আর সবচেয়ে বড় কথা - তারা এখনও সেই একই নটোবট সিস্টেম ব্যবহার করছে যেটা দুই বছর আগে সেটআপ করেছিল।

কিন্তু স্কেল বাড়ার সাথে সাথে নতুন চ্যালেঞ্জ এসেছে। এই চ্যাপ্টারে আমরা দেখব কীভাবে নির্ভর কমিউনিকেশন সেই চ্যালেঞ্জগুলো সামলেছে, কী কী পরিবর্তন করেছে, আর কীভাবে নটোবট তাদের এই স্কেলিং জার্নিতে সাহায্য করেছে।
...
- নেমিং কনভেনশন: `R-MIR-CORE-01` (কোনো জোন প্রিফিক্স ছিল না)
...
শুধু বিটিসিএল আর সামিট না, এখন আরো যুক্ত হয়েছে:
...
সিম্পল দুটো পপ, কোনো হায়ারারকি নেই। Nautobot 3.0 এর হায়ারারকিকাল লোকেশন ফিচার ইউজ করা হয়নি।

### নতুন স্ট্রাকচার (২০২৬):

Nautobot 3.0 এর হায়ারারকিকাল লোকেশন ব্যবহার করে:
...
এই হায়ারার্কি তৈরি করতে প্রথমে লোকেশন টাইপ ডিফাইন করা হয়েছে:
...
এটা ভালো কাজ করছিল যখন দুটো সাইট ছিল। কিন্তু এখন আটটা সাইট। তাই নেমিং আপডেট করা হয়েছে জোন প্রিফিক্স যোগ করে:
...
প্রায় ২০টা ডিভাইস হওয়ায় ইউআই থেকে এক একটা করে ম্যানুয়ালি এডিট করা হয়েছে। বড় স্কেলে (১৫০+ ডিভাইস) এপিআই/স্ক্রিপ্ট দিয়ে বাল্ক রিনেম করা ভালো (পরবর্তী চ্যাপ্টারে দেখব)।
...
এই সব ভিল্যান নটবটে সিএসভি দিয়ে বাল্ক ইমপোর্ট করা হয়েছে।
...
নির্ভর কমিউনিকেশন একটা প্র্যাগম্যাটিক পলিসি বানিয়েছে:
...
- এনওসি এর ইন্টারনাল প্যাচিং
- টেম্পোরারি টেস্ট ক্যাবল
- ২ মিটারের কম প্যাচ কেবল
...
১. **Network Engineer** নতুন সাইট প্ল্যান করে নটবটে:
...
৩. **Network Engineer** নটোবট আপডেট করে:
...
নির্ভর কমিউনিকেশন প্রতি সপ্তাহে একটা ডেটা অডিট করে:
...
প্রতি সোমবার সকালে একটা পাইথন স্ক্রিপ্ট চলে যা ভ্যালিডেট করে এবং রিপোর্ট ইমেইল করে:
...
টিম লিডার এই রিপোর্ট দেখে অ্যাকশন নেন।

## অষ্টম চ্যালেঞ্জ: Performance এবং Response Time

১৫০+ ডিভাইস, ৮টা সাইট, হাজার হাজার আইপি - নটোবট ইউআই স্লো হয়ে যাচ্ছিল না তো?

### Performance Optimization

নির্ভর কমিউনিকেশন কিছু অপ্টিমাইজেশন করেছে:
...
বারবার একই কমপ্লেক্স কোয়েরি না চালিয়ে Saved Filters ব্যবহার।
...
এই অপ্টিমাইজেশন এর পরে রেসপন্স টাইম ভালো আছে। ডিভাইস লিস্ট লোড হতে ২-৩ সেকেন্ড লাগে যা অ্যাকসেপ্টেবল।
...
- লোকেশন হায়ারারকি ডিজাইন (Region → Zone → Cluster → POP)
- লোকেশন টাইপ তৈরি
- নতুন Clusters তৈরি (Mirpur, Uttara Clusters)
- নেমিং কনভেনশন আপডেট (জোন প্রিফিক্স যোগ)
...
- Permission structure review এবং নতুন গ্রুপ তৈরি
...
- কেবল ম্যানেজমেন্ট পলিসি ফাইনালাইজ

**মাস ৪ (এপ্রিল ২০২৫):**

- বনানী পপ যুক্ত
- বারিধারা পপ যুক্ত
- টিম ট্রেনিং (নতুন ফিল্ড অপারেশন মেম্বার  দের)
- এনওসি টিম এক্সপ্যানশন
...
- VLAN স্ট্রাকচার রিডিজাইন এবং সিএসভি ইমপোর্ট
- মাইগ্রেশন প্ল্যানিং এবং এক্সিকিউশন
...
- পারফরম্যান্স টিউনিং (PostgreSQL, pagination)
- অটোমেশন স্ক্রিপ্টs তৈরি (validation, reports)
- টিম ট্রেনিং কমপ্লিট
...
-এনওসি কোয়েরি রেসপন্স টাইম: ৮-১০ মিনিট
- ম্যানুয়াল কাজ: ৮৫%
...
-এনওসি কোয়েরি রেসপন্স টাইম: ৩০ সেকেন্ড
- ম্যানুয়াল কাজ: ২৫%

## শিক্ষা এবং পরামর্শ

নির্ভর কমিউনিকেশনের সিটিও জাহাঙ্গীর সাহেবের কিছু পরামর্শ:
...
"আমাদের একবার নাম চেঞ্জ করতে হয়েছে। প্রথমবারই যদি স্কেলেবল কনভেনশন (জোন প্রিফিক্স সহ) করতাম, এত ঝামেলা হতো না।"

**৩. Data quality এ কম্প্রোমাইজ করবেন না**

"একটা ভুল ডেটা এন্ট্রি পুরো নেটওয়ার্কে সমস্যা করতে পারে। উইকলি অডিট করুন, অটোমেটেড ভ্যালিডেশন রাখুন।"

**৪. টিমকে সাথে নিন**

"নটোবট শুধু একজনের জিনিস না। পুরো টিম যদি ইউজ না করে, তাহলে ডেটা আউটডেটেড হয়ে যায়। প্রপার ট্রেনিং দিন, ডকুমেন্টেশন রাখুন।"
...
- ডিভাইস নেমিং স্কেল করা (জোন প্রিফিক্স যোগ)
...
নির্ভর কমিউনিকেশন এখন ৫০ হাজার কাস্টমারে স্ট্যাবল। তাদের লক্ষ্য পরের দুই বছরে (২০২৮) ১ লক্ষ কাস্টমার। সেজন্য তারা এখন অটোমেশনের দিকে যাচ্ছে। পরের চ্যাপ্টারে আমরা দেখব কীভাবে এপিআই এবং পাইথন দিয়ে নটবটকে অটোমেট করা যায়।
