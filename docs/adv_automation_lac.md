নির্ভর কমিউনিকেশন এখন ৫০ হাজার কাস্টমারে স্ট্যাবল। নটবট দিয়ে নেটওয়ার্ক ডকুমেন্ট করছে, PyNautobot দিয়ে বেসিক অটোমেশন করছে। কিন্তু তাদের লক্ষ্য আরো বড় - পরের দুই বছরে ১ লক্ষ কাস্টমার।
...
Ansible হলো একটা অটোমেশন টুল যা দিয়ে নেটওয়ার্ক ডিভাইস কনফিগার করা যায়। এটার সাথে নটবট integration করলে পাওয়ারফুল কম্বিনেশন হয়।
...
এখন Ansible এর কাছে নটবটে যত ডিভাইস আছে সব দেখতে পাবে:
...
Ansible এখন নটবট থেকে সব কোর রাউটারের লিস্ট নেবে, তারপর প্রতিটাতে NTP কনফিগার করবে।
...
### নটবট + Ansible Workflow

```
১. নটবটে নতুন ডিভাইস add করুন
...
### নটবট Golden Config Plugin

নটবটের একটা plugin আছে - `nautobot-golden-config`।
...
এখন নটবট UI তে **Golden Config** মেনু দেখবেন।
...
নটবট শুধু ডকুমেন্টেশন না - এটা monitoring এর সাথেও integrate করা যায়।

### ধারণা

নটবট থেকে ডিভাইস লিস্ট নিয়ে Prometheus/Zabbix/LibreNMS এ automatically add করা যায়।
...
এই স্ক্রিপ্ট চালালে নটবট থেকে সব ডিভাইস নিয়ে Prometheus এ add করবে। Prometheus restart করলেই monitoring শুরু হবে।
...
এখন নটবটে নতুন ডিভাইস add করলে ১ ঘন্টার মধ্যে automatically monitoring এ যুক্ত হবে!

---

## স্কেলিং মাইলস্টোন - ১০k থেকে ১০০k এর যাত্রা

নির্ভর কমিউনিকেশনসের জার্নি:
...
- সব change নটবট দিয়ে ট্র্যাক করতে হবে
...
একটা chatbot যেটা নটবট API access করতে পারে:
...
এরকম chatbot তৈরি করা যায় LangChain দিয়ে। নটবট API কে function calling এর মাধ্যমে integrate করতে হবে।
...
নির্ভর কমিউনিকেশন এখন একটা solid automation foundation পেয়েছে। তারা জানে কীভাবে ১ লক্ষ কাস্টমারে স্কেল করতে হবে। নটবট শুধু একটা documentation tool না - এটা তাদের পুরো network operations এর হৃদয়।

এই বইয়ের শেষে এসে আমরা দেখলাম কীভাবে একটা ছোট ISP (৫ হাজার কাস্টমার) একটা মিডিয়াম ISP (৫০ হাজার) হয়ে বড় ISP (১ লক্ষ+) এর দিকে যাত্রা করতে পারে। আর সেই যাত্রায় Network Source of Truth (নটবট) হলো মূল ভিত্তি।
