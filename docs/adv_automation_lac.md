নির্ভর কমিউনিকেশন এখন ৫০ হাজার কাস্টমারে স্ট্যাবল। নটোবট দিয়ে নেটওয়ার্ক ডকুমেন্ট করছে, পাইনটবট দিয়ে বেসিক অটোমেশন করছে। কিন্তু তাদের লক্ষ্য আরো বড় - পরের দুই বছরে ১ লক্ষ কাস্টমার।
...
আনসিবল হলো একটা অটোমেশন টুল যা দিয়ে নেটওয়ার্ক ডিভাইস কনফিগার করা যায়। এটার সাথে নটোবট integration করলে পাওয়ারফুল কম্বিনেশন হয়।
...
- name: Configure NTP on all routers
  hosts: core_router  # নটোবট থেকে "Core Router" role এর ডিভাইস
  gather_facts: no
...
আনসিবল এখন নটোবট থেকে সব কোর রাউটারের লিস্ট নেবে, তারপর প্রতিটাতে এনটিপি কনফিগার করবে।
...
নটোবট রিস্টার্ট করুন।

### গোল্ডেন কনফিগ সেটআপ

এখন নটোবট UI তে **গোল্ডেন কনফিগ** মেনু দেখবেন।
...
## মনিটরিং ইন্টিগ্রেশন - নটোবট + প্রমিথিউস/গ্রাফানা

নটোবট শুধু ডকুমেন্টেশন না - এটা মনিটরিং এর সাথেও ইন্টিগ্রেট করা যায়।

### ধারণা

নটোবট থেকে ডিভাইস লিস্ট নিয়ে প্রমিথিউস/জ্যাবিক্স/লিব্রে এনএমএস এ অটোমেটিকালি অ্যাড করা যায়।
...
এই স্ক্রিপ্ট চালালে নটোবট থেকে সব ডিভাইস নিয়ে প্রমিথিউস এ অ্যাড করবে। প্রমিথিউস রিস্টার্ট করলেই মনিটরিং শুরু হবে।
...
- নটোবট সার্ভার: ১৬ জিবি র‍্যাম, ৮ কোরস
...
- সব চেঞ্জ নটোবট দিয়ে ট্র্যাক করতে হবে
...
একটা চ্যাটবট যেটা নটোবট এপিআই অ্যাক্সেস করতে পারে:
...
এরকম চ্যাটবট তৈরি করা যায় ল্যাংচেইন দিয়ে। নটোবট এআই কে ফাংশন কলিং এর মাধ্যমে ইন্টিগ্রেট করতে হবে।
...
নির্ভর কমিউনিকেশন এখন একটা সলিড অটোমেশন ফাউন্ডেশন পেয়েছে। তারা জানে কীভাবে ১ লক্ষ কাস্টমারে স্কেল করতে হবে। নটোবট শুধু একটা ডকুমেন্টেশন টুল না - এটা তাদের পুরো নেটওয়ার্ক অপারেশনস এর হৃদয়।

এই বইয়ের শেষে এসে আমরা দেখলাম কীভাবে একটা ছোট আইএসপি (৫ হাজার কাস্টমার) একটা মিডিয়াম আইএসপি (৫০ হাজার) হয়ে বড় আইএসপি (১ লক্ষ+) এর দিকে যাত্রা করতে পারে। আর সেই যাত্রায় নেটওয়ার্ক সোর্স অফ ট্রুথ  (নটোবট) হলো মূল ভিত্তি।
