আগের চ্যাপ্টারে আমরা Nautobot 3.0 ইনস্টল করলাম। নির্ভর কমিউনিকেশনের NOC টিম এখন একটা চালু নটবট instance পেয়েছে। কিন্তু এখনও কোনো ডেটা নেই - খালি একটা সিস্টেম। ডেটা এন্ট্রি শুরু করার আগে বুঝতে হবে নটবট কীভাবে নেটওয়ার্ক ডেটা স্টোর করে, কীভাবে বিভিন্ন জিনিস একে অপরের সাথে সম্পর্কিত।
...
Organization App হলো নটবটের ভিত্তি। এখানে আপনার নেটওয়ার্কের organizational structure define করবেন।
...
DCIM (Data Center Infrastructure Management) হলো নটবটের সবচেয়ে বড় অংশ। এখানে আপনার সব physical network equipment থাকবে।

#### Device Types - ব্লুপ্রিন্ট

কোনো device যোগ করার আগে তার "type" define করতে হয়। Device Type হলো একটা blueprint বা template।

নির্ভর কমিউনিকেশন MikroTik router ব্যবহার করে। তাদের main model হলো CCR2116-12G-4S+।
...
নির্ভর কমিউনিকেশনের Mirpur POP এ একটা core router আছে:
...
নির্ভর কমিউনিকেশনের convention:
```
...
Cable তৈরি করার পর নটবটের "Trace" ফিচার দিয়ে পুরো physical path follow করতে পারবেন। যেমন একটা customer এর connection কোন কোন device দিয়ে গেছে - সব দেখা যায়।
...
নটবটে একটা visual rack elevation দেখা যায়। কোন U position এ কোন device আছে - graphically দেখায়।
...
নির্ভর কমিউনিকেশনের কাছে BTCL থেকে পাওয়া একটা /22 public IP block আছে:
...
নির্ভর কমিউনিকেশনের VLAN structure:
...
যেকোনো ISP এর নিজের uplink provider থাকে। নির্ভর কমিউনিকেশন BTCL থেকে ইন্টারনেট নেয়।
...
নির্ভর কমিউনিকেশনের একটা secondary provider ও আছে:
...
এভাবে নটবট জানে ঠিক কোন interface এ uplink আছে।

---

### Extras App - Flexibility এবং Customization

Extras app হলো নটবটের "toolbox"। এখানে এমন জিনিস আছে যা সব apps এ ব্যবহার হয়।
...
নির্ভর কমিউনিকেশন এর Device Roles:
...
নির্ভর কমিউনিকেশন একটা custom status যোগ করেছে:
...
নটবটে যদি কোনো built-in field না থাকে, custom field তৈরি করুন।

নির্ভর কমিউনিকেশনের দরকার warranty tracking:
...
এই পুরো structure নটবটের ডেটাবেসে store করা। এখন যদি কেউ জিজ্ঞাসা করে:
...
### ডেটা ইন্টিগ্রিটি - নটবট কীভাবে নিশ্চিত করে

নটবট অনেক validation করে যাতে ভুল ডেটা ঢুকতে না পারে।

#### Referential Integrity

যদি একটা Location মুছতে চান যেখানে Devices আছে, নটবট বলবে:
