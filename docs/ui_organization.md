...
নির্ভর কমিউনিকেশনের NOC টিম এখন তাদের মিরপুর এবং উত্তরা পপের সম্পূর্ণ নেটওয়ার্ক নটবট ডকুমেন্ট করবে। তাদের সাথে একসাথে আমরাও শিখব। শুরু করা যাক।

### Nautobot 3.0 UI Overview

প্রথমে একবার নটবট UI এর সাথে পরিচিত হয়ে নিই।
...
ডান দিকে উপরের কোণায় একটা সার্চ বক্স দেখবেন। এটা গ্লোবাল সার্চ - যেকোনো কিছু এখানে টাইপ করলে নটবট সব জায়গায় খুঁজবে।
...
**Create and অ্যাড Another** ক্লিক করুন।
...
**Create and অ্যাড Another** ক্লিক করুন (যদি আরো র‍্যাক যোগ করতে চান)।
...
নটবটে CSV দিয়ে bulk import করার সুবিধা আছে। চলুন দেখি কীভাবে।
...
- Device Type, Role, Location, Rack এর নাম হুবহু নটবটে যেভাবে লেখা আছে সেভাবে লিখতে হবে
...
নটবট CSV parse করবে এবং একটা preview দেখাবে:
...
নির্ভর কমিউনিকেশনের নেটওয়ার্কে বিভিন্ন ধরনের ট্রাফিক আছে - ম্যানেজমেন্ট, কাস্টমার, আপলিংক। এগুলো আলাদা করার জন্য VLAN ব্যবহার করা হয়।
...
এখন নটবট জানে 10.10.10.0/24 prefix টা VLAN 10 (MGMT_MIRPUR) এ ব্যবহার হয়।
...
নটবটে built-in cable types আছে:
...
নির্ভর কমিউনিকেশনের upstream connectivity BTCL থেকে। চলুন এটা ডকুমেন্ট করি।
...
নির্ভর কমিউনিকেশনের NOC টিম প্রথমে কিছু ভুল করেছিল। চলুন সেগুলো দেখি যাতে আপনি এড়াতে পারেন।
...
নটবটে সব cable update করা হয়েছে এই color scheme অনুযায়ী।
...
নির্ভর কমিউনিকেশন একটা documented naming convention তৈরি করেছে। এটা একটা Word ডকুমেন্টে রাখা আছে এবং সব team members কে দেওয়া হয়েছে।
...
নির্ভর কমিউনিকেশন প্রতি মাসের ১ তারিখে একটা snapshot নেয়:
...
নির্ভর কমিউনিকেশন এখন তাদের মিরপুর এবং উত্তরা পপের প্রায় সম্পূর্ণ নেটওয়ার্ক নটবটে ডকুমেন্ট করে ফেলেছে। পরের চ্যাপ্টারে আমরা দেখব কীভাবে এই setup কে scale করা যায় যখন নতুন পপ যুক্ত হয়, আরো ডিভাইস আসে, এবং কীভাবে advanced features ব্যবহার করতে হয়।
